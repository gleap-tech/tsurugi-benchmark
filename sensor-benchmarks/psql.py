...
PgRowType = tuple[float, float, float, datetime]
...
) -> list[list[list[PgRowType]]]:
    base_time = datetime.now(timezone.utc)
    all_workers: list[list[list[PgRowType]]] = []
    seq = 0

    for _ in range(n_workers):
        worker_commits: list[list[PgRowType]] = []
        for _ in range(n_commits_per_worker):
            rows_in_one_commit: list[PgRowType] = []
...
async def insert_worker(table: str, commit_rows_list: list[list[PgRowType]]) -> None:
    sql = f"""
        INSERT INTO {table}
...
async def write_parallel(
    table: str,
    all_worker_rows: list[list[list[PgRowType]]],
) -> float:
    start = time.perf_counter()
    await asyncio.gather(
        *[insert_worker(table, worker_rows) for worker_rows in all_worker_rows]
    )
...
async def run_rounds() -> None:
    ...
    for i in range(WARMUP_ROUNDS):
        ...
        elapsed = await write_parallel(table, all_worker_rows)
